"""
Database — Grupbalar, abonentler we blok journaly.
SQLite ulanylýar (başlangyç üçin ýeterlik, soňra PostgreSQL geçip bolar).
"""
...
class Database:
    ...
    def log_action(self, chat_id: int, user_id: int, username: str,
        ...
        with self._conn() as conn:
            conn.execute("""
                INSERT INTO ban_log
                    (chat_id, user_id, username, reason, confidence, message_preview)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (chat_id, user_id, username, reason, confidence, message_preview))
    ...
    def get_stats(self, chat_id: int) -> dict:
        with self._conn() as conn:
            ...
            scam = conn.execute(
                "SELECT COUNT(*) FROM ban_log WHERE chat_id = ? AND reason LIKE '%scam%'",
                (chat_id,)
            ).fetchone()[0]
            sexual = conn.execute(
                "SELECT COUNT(*) FROM ban_log WHERE chat_id = ? AND reason LIKE '%seks%'",
                (chat_id,)
            ).fetchone()[0]
        return {"total_bans": total, "scam_bans": scam, "sexual_bans": sexual}